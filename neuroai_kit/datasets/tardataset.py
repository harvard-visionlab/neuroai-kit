...
class TarDataset(Dataset):
    ...
    def filter_samples(self, is_valid_file=None, extensions=(".png", ".jpg", ".jpeg")):
        ...
        if is_valid_file is None:
            ...
            def is_valid_file(m):
                is_valid = (
                    m.isfile()
                    and m.name.lower().endswith(extensions)
                    and ".ipynb_checkpoints" not in m.name.lower()
        ...
        self.samples = [
            m.name for m in self.members_by_name.values() if is_valid_file(m)
        ]
    ...
    def __getitem__(self, index):
        ...
        image = self.get_image(filename, pil=True)
